    ...
    <header class="bloco-conteudo">
        ...
        <div class="flex-space-between">
            ...
            <div class="flex-centro">
                <a href="<?php echo HOME ?>/painel/diarios-turma/<?php echo $dados->id_turma ?>" class="botao-circular botao-circular-padrao-claro botao-circular-medio margin-right10">
                    <div><span class="material-icons icone">keyboard_arrow_left</span></div>
                </a>

                <div class="titulo font-bold texto-cor-dourado">Diários da Turma: <?php echo SeriesModel::find_by_id($dados->turma->id_serie)->nome .' '. $dados->turma->turma.' '.($dados->turma->turno == 'manha' ? 'Manhã' : 'Tarde') ?></div>
            </div>
    ...
    <section class="bloco-conteudo">
        ...
        <div class="table-responsive">
            <table class="tabela-linha-ativa">
                ...
                <?php
                if(!empty($dados->diarios)):
                    foreach ($dados->diarios as $registro):

                        echo '
                            <tr>
                                <td width="50">'.$registro->data->format('d/m/Y').'</td>
                                <td class="flex-a-direita">
                                    <a data-tippy-content="Visualizar Diários" href="'.HOME.'/painel/alterar-diario-turma/'.$registro->id_turma.'/'.$registro->id_materia.'/'.$registro->id.'" class="botao-circular botao-circular-padrao-claro botao-circular-pequeno margin-right10"><i class="material-icons icone">visibility</i></a>
